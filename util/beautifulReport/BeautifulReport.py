...
class PATH:
    """ all file PATH meta """
    config_tmp_path = FileUtil().getProjectObsPath() + '/util/beautifulReport/template/template'
...
class ReportTestResult(unittest.TestResult):
    ...
    def success_rate(self):
        ...
        :return: str 通过率 100%
        """
        all_case = len(self.result_list)
        success_case = self.success_count
        rate = float(success_case)/float(all_case) * 100
        success_case = str('%.2f' % rate) + '%'
        return success_case
